...
#fst <- bc_fst
genes <- "output/chest_genes_of_interest.csv"
...
manc.labels<-function(fst, genes){
  ...
  genes.interest <- read.csv(genes, header = TRUE) %>% distinct(blast_name, .keep_all = TRUE) %>% 
  ...
  genes.fst <- genes.fst %>% select(seqid, start, chr, blast_name, Name, row, fst) %>% 
    group_by(Name) %>% filter(fst == max(fst))
  
  genes.fst <- genes.fst %>% mutate(fst = ifelse(blast_name == "Lgr6", 0.482, fst))

  
  better_label <- genes.fst %>% mutate(seqid2 = as.numeric(seqid)) %>% 
    group_by(seqid) %>% 
  ...
  better_label <- better_label %>% 
    dplyr::mutate(fst = ifelse(fst < min(olf3$fst), max(fst), fst))
  ...
  fst_main <- fst %>% 
    ...
    theme(legend.position="none",
          ...
          axis.ticks.x=element_blank(),
          axis.ticks.y=element_line(size=0.2)) +
    geom_text_repel(data = better_label, aes(x = row, y = fst, label = blast_name), 
                    color = "black", 
                    nudge_y = 0.1, 
                    direction='x',
                    segment.size=0.2,
